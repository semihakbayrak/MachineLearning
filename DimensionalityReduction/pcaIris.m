...
mTotal = zeros(4,1);
for i=1:N
    mTotal = mTotal + (Xtrain(i,:))';
end
mTotal = mTotal/N; %mean
STotal = zeros(4,4);
for i=1:N
    STotal = STotal + ((Xtrain(i,:))'-mTotal)*((Xtrain(i,:))'-mTotal)';    
end
STotal = STotal / N;
...
hold on
bound1z1 = zeros(1,101);
bound1z2 = zeros(1,101);
bound1extraz1 = zeros(1,101);
bound1extraz2 = zeros(1,101);
bound2z1 = zeros(1,101);
bound2z2 = zeros(1,101);
bound2extraz1 = zeros(1,101);
bound2extraz2 = zeros(1,101);

for a=101:-1:1
    for b=1:101
        if posterior((a-1)/10,(b-1)/10,1,1,0)>posterior((a-1)/10,(b-1)/10,2,1,0)
            bound1z1(102-a) = (a-1)/10;
            bound1z2(102-a) = (b-1)/10;
            break
        end
    end
    for b=101:-1:1
        if posterior((a-1)/10,(b-1)/10,1,1,0)>posterior((a-1)/10,(b-1)/10,2,1,0)
            bound1extraz1(102-a) = (a-1)/10;
            bound1extraz2(102-a) = (b-1)/10;
            break
        end
    end
end
for a=101:-1:1
    for b=1:101
        if posterior((a-1)/10,(b-1)/10,2,1,0)>posterior((a-1)/10,(b-1)/10,3,1,0)
            bound2z1(102-a) = (a-1)/10;
            bound2z2(102-a) = (b-1)/10;
            break
        end
    end
    for b=101:-1:1
        if posterior((a-1)/10,(b-1)/10,2,1,0)>posterior((a-1)/10,(b-1)/10,3,1,0)
            bound2extraz1(102-a) = (a-1)/10;
            bound2extraz2(102-a) = (b-1)/10;
            break
        end
    end
end
...
for a=101:-1:1
    for b=1:101
        if posterior((a-1)/10,(b-1)/10,1,4,Scommon)>posterior((a-1)/10,(b-1)/10,2,4,Scommon)
            bound1z1(102-a) = (a-1)/10;
            bound1z2(102-a) = (b-1)/10;
            break
        end
...
for a=101:-1:1
    for b=1:101
        if posterior((a-1)/10,(b-1)/10,2,4,Scommon)>posterior((a-1)/10,(b-1)/10,3,4,Scommon)
            bound2z1(102-a) = (a-1)/10;
            bound2z2(102-a) = (b-1)/10;
            break
        end
